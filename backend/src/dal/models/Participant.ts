...
export const addParticipant = async (meetingId: string, userId: string): Promise<Participant> => {
    const sql = `
        INSERT INTO participants (meeting_id, user_id, status)
        VALUES ($1, $2, $3)
        RETURNING *
    `;
    const result = await query(sql, [meetingId, userId, ParticipationStatus.PENDING]);
...
export const getUserInvitations = async (userId: string): Promise<any[]> => {
    const sql = `
        SELECT p.*, m.title, m.start_time, m.end_time, m.address, u.first_name as organizer_first_name, u.last_name as organizer_last_name
        FROM participants p
...
export const removeParticipant = async (meetingId: string, userId: string): Promise<boolean> => {
