    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (newMeeting.address && showCreateModal) {
                const alreadySelected = suggestions.some(s => s.display_name === newMeeting.address);
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                latitude: newMeeting.latitude ? parseFloat(String(newMeeting.latitude)) : undefined,
                longitude: newMeeting.longitude ? parseFloat(String(newMeeting.longitude)) : undefined
            };
            ...
            setNewMeeting({ title: '', description: '', startTime: '10:00', endTime: '11:00', address: '', latitude: '', longitude: '' });
            fetchMeetings();
        } catch (error: any) {
        ...
        <div className="dashboard">
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-muted)' }}>Manage your schedule and locations</p>
                </div>
                <div style={{ display: 'flex', gap: '1rem' }}>
                    <button onClick={() => setShowCreateModal(true)} className="btn-primary" style={{ width: 'auto', padding: '0.75rem 1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '2rem', alignItems: 'start' }}>
                <aside>
                    ...
                    />
                    <div style={{ marginTop: '2rem', padding: '1.5rem', background: 'var(--bg-card)', borderRadius: '1rem', border: '1px solid var(--border)' }}>
                        <h3 style={{ marginBottom: '1rem' }}>Today's Overview</h3>
                        <p style={{ fontSize: '2.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>{meetingsOnSelectedDate.length}</p>
                ...
                <main>
                    ...
                    <div className="meeting-grid" style={{ gridTemplateColumns: '1fr', marginTop: 0 }}>
                        {meetingsOnSelectedDate.length === 0 ? (
                            <div style={{ padding: '3rem', textAlign: 'center', background: 'var(--bg-card)', borderRadius: '1rem', border: '1px dashed var(--border)' }}>
                                <p style={{ color: 'var(--text-muted)' }}>No meetings for this day.</p>
                            </div>
                        ) : (
                            meetingsOnSelectedDate.map((meeting: Meeting) => (
                                <div key={meeting.id} className="meeting-card" onClick={() => setSelectedMeeting(meeting)}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                                        ...
                                        <button style={{ background: 'rgba(99, 102, 241, 0.1)', color: 'var(--primary)', padding: '0.5rem 1rem', fontSize: '0.75rem' }}>View Details</button>
                                    </div>
                                </div>
                            ))
                        )}
                    ...
                    <div className="card" style={{ maxWidth: '500px', padding: '2rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem' }}>
                            <div>
                                ...
                                <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>For {date.toDateString()}</p>
                            </div>
                            <button style={{ background: 'transparent', fontSize: '1.5rem', color: 'var(--text-white)' }} onClick={() => setShowCreateModal(false)}>&times;</button>
                        </div>
                        <form onSubmit={handleFormSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            <input type="text" placeholder="Title" value={newMeeting.title} onChange={e => setNewMeeting({...newMeeting, title: e.target.value})} required />
                            <textarea 
                                placeholder="Description" 
                                value={newMeeting.description} 
                                onChange={e => setNewMeeting({...newMeeting, description: e.target.value})}
                                style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-white)', padding: '0.75rem', borderRadius: '8px', minHeight: '80px' }}
                            />
                            <div style={{ display: 'flex', gap: '1rem' }}>
                                <div style={{ flex: 1 }}>
                                    <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Start Time</label>
                                    <input type="time" style={{ width: '100%' }} value={newMeeting.startTime} onChange={e => setNewMeeting({...newMeeting, startTime: e.target.value})} required />
                                </div>
                                <div style={{ flex: 1 }}>
                                    <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>End Time</label>
                                    <input type="time" style={{ width: '100%' }} value={newMeeting.endTime} onChange={e => setNewMeeting({...newMeeting, endTime: e.target.value})} required />
                                </div>
                            </div>
                            <div style={{ position: 'relative' }}>
                                <input 
                                    ...
                                    autoComplete="off"
                                />
                                {isSearching && <div style={{ position: 'absolute', right: '10px', top: '10px', fontSize: '0.75rem', color: 'var(--primary)' }}>Searching...</div>}
                            ...
                            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', opacity: 0.6 }}>
                                ...
                                <span style={{ fontSize: '0.75rem' }}>
                                    {newMeeting.latitude ? `Coordinates: ${newMeeting.latitude}, ${newMeeting.longitude}` : 'Select an address to get coordinates automatically'}
                                </span>
                            </div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100 }}>
                    <div className="card" style={{ maxWidth: '600px', padding: '2rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
                            ...
                            <button style={{ background: 'transparent', fontSize: '1.5rem', color: 'var(--text-white)' }} onClick={() => setSelectedMeeting(null)}>&times;</button>
                        </div>
                        <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem' }}>{selectedMeeting.description}</p>
                        ...
                        {getGoogleMapsEmbedUrl(selectedMeeting.latitude, selectedMeeting.longitude, selectedMeeting.address) && (
                            <iframe 
                                className="map-embed"
                                title="meeting-location"
                                loading="lazy"
                                allowFullScreen
                                src={getGoogleMapsEmbedUrl(selectedMeeting.latitude, selectedMeeting.longitude, selectedMeeting.address)}
                            ></iframe>
                        )}

                        <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem' }}>
                            <button className="btn-primary" onClick={() => startEditing(selectedMeeting)}>Edit</button>
                            <button className="btn-primary" style={{ background: 'var(--error)' }} onClick={() => handleDeleteMeeting(selectedMeeting.id)}>Delete</button>
